...
def test_srwkv_layer():
    ...
    binary_check = torch.logical_or(torch.isclose(output, torch.zeros_like(output)), 
    ...
    for test_seq_len in [1, 8, 16]:
        ...
        print(f"Seq len {test_seq_len}: input {test_x.shape} → output {test_output.shape}")
    ...
    for test_batch_size in [1, 4, 8]:
        ...
        test_output = head(test_x)
        print(f"Batch size {test_batch_size}: input {test_x.shape} → output {test_output.shape}")
